...
def plot_GP_1D(X_train, Y_train, lin, mean, var):
    ...
    plt.plot(lin, mean, color="black", linestyle="-", label="GP")
    plt.plot(lin, mean + 2 * var, color="Navy")
    plt.plot(lin, mean - 2 * var, color="Navy")
    plt.fill_between(lin.ravel(), (mean + 2 * var).ravel(),
                     (mean - 2 * var).ravel(),
...
def plot_improvement(X_train, Y_train, grid, mean, var, improvement):
    ...
    ax[0].scatter(X_train, Y_train, marker="x", color="red", label="Train")
    ax[0].plot(grid, mean, linestyle="--", color="black", label="GP")
    ax[0].plot(grid, mean + 2 * var, alpha=0.3, color="navy")
    ax[0].plot(grid, mean - 2 * var, alpha=0.3, color="navy")
    ax[0].fill_between(grid.ravel(), (mean + 2 * var).ravel(),
                       (mean - 2 * var).ravel(),
                       alpha=0.2, color="navy", label="confidence")

    ax[1].plot(grid, improvement, alpha=0.3, color="orange")
    ax[1].fill_between(grid.ravel(), improvement.ravel(),
                       np.full(grid.ravel().shape, 0), color="orange",
                       alpha=0.2, label="EI")
    return plt
