...
def test_GP_1D(optimize=False):
    ...
    if optimize:
        new = gp.grid_search_optimization(constrains=[[1, 30], [1, 30]],
                                          n_points=300, function=np.random.uniform)
        ...
        print("New marg likelihood :", optimized.get_marg(), "\n Hyperparameters: ",
              optimized.get_kernel().gethyper())


def test_GP_2D(optimize=False, function=np.linspace):
    dim_test = 2
    dim_out = 1
    n_train_p = 7
...
def test_minimization_2D():
    ...
    # X=np.array([[3,-2]])
    """def f(X):
        return ((X[:, 1] ** 2 * X[:, 0] ** 2) +((X[:, 1] ** 2 - X[:, 0] ** 2)))
        
        def f(X):
        return (X[:, 0] + 2 * X[:, 1] - 7) ** 2 + (2 * X[:, 0] + X[:, 1] - 5) ** 2

    def f(X):
       return (X[:, 1] ** 2 * X[:, 0] ** 2) * np.sin((X[:, 1] ** 2 + X[:, 0] ** 2))"""
    ...
    """def f(X):
        return - (X[:,1]+ 47)*np.sin(np.sqrt(np.abs(
            X[:,1]+X[:,0]/2+47)))- X[:,0]* np.sin(
    ...
    best = BayOpt.bayesian_run_min(400, boundaries, iteration=100,
                                   optimization=True, epsilon=0.01, func=np.random.uniform)
    print("bay:", best)
    """ best = BayOpt.bayesian_run(100, boundaries, iteration=10, optimization=True)
    print("bay:", best)"""
...
def test_minimization_1D():
    ...
    BayOpt = BayesianOptimization(X, Z, gp, f)
    # best=BayOpt.bayesian_run(100,  [[-1,4] for i in range(dim_test)] , iteration=30, optimization=False)
    best = BayOpt.bayesian_run_BFGL(250, [[-3, 3]], iteration=10,
                                   optimization=True, opt_constrain=[[1, 20], [2, 20]],
                                   epsilon=0.1, func=np.linspace)
    print("bay:", best)
...
def test_Hartmann_6D():
    ...
    # best=BayOpt.bayesian_run(100,  [[-1,4] for i in range(dim_test)] , iteration=30, optimization=False)
    n_p = 15
    best = BayOpt.bayesian_run_min(n_p, [[0, 1] for i in range(6)], iteration=50, optimization=True)
    print("Number of points sampled in an iteration: ", n_p ** dim)
    print("bay:", best)

def test_min_gauss(dim):
    boundaries=[[-3,3] for i in range(dim)]
    mean=[0]*dim
    cov=np.eye(dim)*2
    x=np.random.uniform(-3,3, (1,dim))
    ...
    # best=BayOpt.bayesian_run(100,  [[-1,4] for i in range(dim_test)] , iteration=30, optimization=False)
    n_p = 3
    best = BayOpt.bayesian_run_min(n_p, boundaries, iteration=50, optimization=True, func=np.random.uniform)
    print("Number of points sampled in an iteration: ", n_p ** dim)
    print("bay:", best)
